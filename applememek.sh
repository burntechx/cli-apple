...
Report any bugs to: KRISNADWI286@GMAIL.COM
"
  exit 1 
}
...
nagahitam_apple_validator() {
  ...
  if [[ $val > 0 || $icl > 0 ]]; then
    printf "[$header] $2/$3. ${GRN}LIVE -> $1 ${CYAN} $footer"
    echo "LIVE => $1" >> $4/live.txt
  else
    if [[ $inv > 0 || $bad > 0 ]]; then
      printf "[$header] $2/$3. ${RED}DIE -> $1 ${CYAN} $footer"
      echo "DIE => $1" >> $4/die.txt
    else
      printf "[$header] $2/$3. ${PUR}UNKNOWN -> $1 ${CYAN} $footer"
...
echo "live: `grep -c "@163" $inputFile`"
echo "live: `grep -c "@icloud" $inputFile`"
echo "live: `grep -c "@juno" $inputFile`"
echo "live: `grep -c "@sbcglobal" $inputFile`"
echo "live: `grep -c "@gmx" $inputFile`"
echo "live: `grep -c "@cox" $inputFile`"
echo "========================================="
...
getKey() {
  ...
  apiKey='6e8a82e92cb00781e3f19847f71ed255'
